...
function saveUser(req, res) {
	...
	if (params.text && params.select) {
		newUser.text = params.text
		newUser.select = params.select
		newUser.opciones = params.opciones
	...
	User.find()
		.then((usuarios) => {
			const filter = usuarios.some(usuario => usuario.text == newUser.text)
			if (filter) {
